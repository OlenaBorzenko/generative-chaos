...
export default function TorusCanvas({ config, width = 600, height = 600 }: TorusCanvasProps) {
    ...
    const sketch = (s: p5) => {
      ...
      const computeKnotPath = () => {
        ...
        for (let i = 0; i <= pathDetail; i++) {
          let t = s.map(i, 0, pathDetail, 0, s.TWO_PI);
          let r = knotRadius + waveAmplitude * s.cos(q * t);
          let x = s.cos(p * t) * r;
          let y = s.sin(p * t) * r;
          let z = 0.5 * s.sin(q * t);
      ...
      const computeRingsWithParallelTransport = () => {
        const {
          ...
          lumps, lumpHeight, lumpOffset, eccentricity
        } = config;
      ...
      const rotateVectorAroundAxis = (v: p5.Vector, axis: p5.Vector, angle: number) => {
      ...
      const drawWireframe = () => {
        ...
        for (let i = 0; i < ringPoints.length; i++) {
          ...
          if (i < ringPoints.length - 2) {
            ...
            for (let j = 0; j < ringPoints[i].length; j++) {
